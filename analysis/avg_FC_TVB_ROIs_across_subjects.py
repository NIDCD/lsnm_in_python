...
# generate a number of random matrices
rand_mat = np.zeros((num_random, 998, 998))
for r in range(0, num_random):
    rand_mat[r] = np.random.rand(998,998)
...
for p in threshold_array: 
    ...
    tvb_rs_mean_a   = bct.threshold_absolute(tvb_rs_mean, 0, copy=True)
    tvb_rs_mean_b   = bct.normalize(tvb_rs_mean_a, copy=True)
    tvb_rs_mean_p   = bct.threshold_proportional(tvb_rs_mean_b, p, copy=True)
